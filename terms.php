...
<head>
<?php 
    $title = "Shimaguni Terms";
 $pageCss = "terms.css";
	?>

<meta name="description" content="Shimaguni Terms and Conditions " />
...
#termsContainer {
	...
	display:block
        ...
        <div id="content">
         ...
         <span class="title-text"> <p>TERMS AND CONDITIONS</p></span>
...
<script>
 ...
 $(function(){
	var items = (Math.floor(Math.random() * ($('.feedback li').length)));
	$('.feedback li').hide().eq(items).show();
	
  function next(){
		$('.feedback li:visible').delay(8000).fadeOut('slow',function(){
			$(this).appendTo('.feedback ul');
			$('.feedback li:first').fadeIn('slow',next);
    });
   }
  next();
});
